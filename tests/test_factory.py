...
def test_create_poetry(new_format: str) -> None:
    ...
    for requires in (package.requires_for_locking, package.all_requires_for_locking):
        tomlkit_for_locking = next(d for d in requires if d.name == "tomlkit")
...
def test_validate_strict_dynamic_warnings() -> None:
    ...
    complete = fixtures_dir / "complete_new_dynamic.toml"
    with complete.open("rb") as f:
        content = tomllib.load(f)
