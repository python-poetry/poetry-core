...
class WheelBuilder(Builder):
    ...
    def _add_pth(self, wheel: zipfile.ZipFile) -> None:
        ...
        # Write a pure-ASCII .pth that imports a helper module. On Python ≤3.12,
        # site.addpackage() reads .pth files with the locale encoding, so embedding
        ...
        with self._write_to_zip(wheel, str(pth_file)) as f:
            f.write(f"import {helper_name}\n")

        path_list = sorted(paths)
        helper_src = (
            "# Auto-generated by poetry-core for editable installs.\n"
            "from __future__ import annotations\n\n"
            "import sys\n\n"
            f"_PATHS = {path_list!r}\n\n"
            "for _path in _PATHS:\n"
            "    if _path not in sys.path:\n"
            "        sys.path.insert(0, _path)\n"
        )
        with self._write_to_zip(wheel, f"{helper_name}.py") as f:
            f.write(helper_src)
